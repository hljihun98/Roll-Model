...
기본 배치는 **2열 × 2행, 4바퀴 전체 접지**입니다. **하중 설정 → 접지 조건 → 3점 접지**에서 바닥에 닿지 않는 바퀴를 선택하면 나머지 3점의 반력과 최대 캐스터 하중을 계산합니다. 바퀴 위치는 배치 중심을 기준으로 전후 x, 좌우 y로 표시합니다. 합력 작용점이 지지 삼각형 밖이면 불가, 경계에 있으면 전도 여유가 없는 주의 상태입니다. 3점 조건은 2×2 배치에서만 지원하며, 상세 계산은 [모델 적용 범위](docs/MODEL-NOTES.md)를 참고하세요.
...
화면과 가이드는 **① 사용 조건 → ② 하중 → ③ 바퀴 → ④ 바닥 → ⑤ 주행·기동 → ⑥ 판정 결과** 순서입니다. **하중 설정**에서 중량 산출 또는 N 직접 입력을 선택하고, 각 그룹의 상세 설정에서 물성을 조절합니다. 판정 요약은 입력 영역 바로 아래에 표시됩니다. 모바일에서는 상세 입력을 필요할 때만 펼치고 하단 **입력 보기 / 판정 결과 보기**로 이동합니다. 가이드의 **실제 설정 위치 보기**는 해당 입력을 강조하며, 상단 **가이드 이어보기**로 같은 단계에 돌아올 수 있습니다.

넓은 화면에서는 **왼쪽 입력·판정 / 오른쪽 접촉 그림**을 나란히 표시하고, 실제 콘텐츠 폭이 좁아지면 한 열로 전환합니다. 접촉 단면과 접촉 자국 그림은 높이를 제한하고 **크게 보기**로 확대할 수 있습니다. 그림 아래의 접힌 정보에서 재료·계산 근거를 확인합니다.

상세 모드에서는 **깊이 방향 응력·표면 응력 분포**도 오른쪽에 이어서 표시합니다. 공간이 충분하면 두 응력 그래프를 나란히 배치하며, 각각 확대 보기와 해석·범례 펼치기를 지원합니다.
