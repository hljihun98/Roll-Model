...
화면과 가이드는 **① 사용 조건 → ② 하중 → ③ 바퀴 → ④ 바닥 → ⑤ 주행·기동 → ⑥ 판정 결과** 순서입니다. **하중 설정**에서 중량 산출 또는 N 직접 입력을 선택하고, 각 그룹의 상세 설정에서 물성을 조절합니다. 판정 요약은 접촉 그림 위에 표시됩니다. 모바일에서는 상세 입력을 필요할 때만 펼치고 하단 **입력 보기 / 판정 결과 보기**로 이동합니다. 가이드의 **실제 설정 위치 보기**는 해당 입력을 강조하며, 상단 **가이드 이어보기**로 같은 단계에 돌아올 수 있습니다.
