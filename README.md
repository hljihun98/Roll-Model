...
주차로봇·AMR 캐스터의 하중, 접촉압, 도막 손상, 발열을 연결해 판정하는 설계 스크리닝 도구입니다. 서버·외부 JavaScript·프레임워크 없이 실행됩니다. 글꼴을 불러오지 못해도 시스템 글꼴로 동작합니다.

**웹 실행:** [hljihun98.github.io/Roll-Model](https://hljihun98.github.io/Roll-Model/)

기본 배치는 **2열 × 2행, 4바퀴 전체 접지**입니다. **상세 조건 → 접지 조건**에서 두 가지 3점 조건을 고를 수 있습니다. **3점 접지 · 구동부 2(전후) + 중앙 캐스터**는 틸팅 구동부(바퀴 2개 = 접지점 1개) 두 개와 캐스터 하나로 서는 로봇이며, 축거는 구동부 간격, 윤거는 구동부 축선에서 캐스터까지의 좌우 거리입니다. **들림 검토**는 2×2 배치에서 바닥에 닿지 않는 바퀴를 선택합니다. 합력 작용점이 지지 삼각형 밖이면 불가, 경계에 있으면 전도 여유가 없는 주의 상태입니다. 상세 계산은 [모델 적용 범위](docs/MODEL-NOTES.md)를 참고하세요.
...
화면과 가이드는 **① 사용 조건 → ② 하중 → ③ 바퀴 → ④ 바닥 → ⑤ 주행·기동 → ⑥ 판정 결과** 순서입니다. **기본 모드**에는 필수 입력(로봇 중량·적재하중·휠 배치·축거·윤거, 바퀴 재질·D·L, 바닥, 속도·기동)만 있어 이것만 넣어도 1차 결과가 나옵니다. 각 입력 옆 칩은 결과 영향도이며 누르면 꼭 알아야 하는 값인지, 값 출처(사용자 입력·기본값·제조사·실측·추정)와 기본값 복귀를 보여 줍니다. **상세 모드**에서 CG·가감속·물성 같은 상세 조건을, 접힌 **전문가 설정**에서 경험계수·열 계수·판정 경계를 조절합니다. 판정 아래 **결과 신뢰도(A~D)** 카드는 기본값 사용 현황, 영향이 큰 기본값 경고, 현재 결과에 영향이 큰 입력 5개(±10% 재계산)를 보여 줍니다. 시나리오는 로봇 400 kg + 적재하중(차량) 조합이며 모두 4점 접지로 시작합니다. 판정 요약은 입력 영역 바로 아래에 표시됩니다. 모바일에서는 상세 입력을 필요할 때만 펼치고 하단 **입력 보기 / 판정 결과 보기**로 이동합니다. 가이드의 **실제 설정 위치 보기**는 해당 입력을 강조하며, 상단 **가이드 이어보기**로 같은 단계에 돌아올 수 있습니다.
...
src/
  ...
  32_inputs.js               입력 중요도·영향도·값 출처·신뢰도·민감도
  40_body.html               마크업
  50_ui.js                   입력·도면·표·저장
